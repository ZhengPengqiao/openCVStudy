...
int main(int argc, char **argv)
{
    ...
    //在窗口中显示载入的图片
    imshow("(CV_LOAD_IMAGE_UNCHANGED)", imageUnchanged);
    imshow("(CV_LOAD_IMAGE_GRAYSCALE)", imageGrayScale);
    imshow("(CV_LOAD_IMAGE_COLOR)", imageColor);
    imshow("(CV_LOAD_IMAGE_ANYDEPTH)", imageAnyDepth);
    imshow("(CV_LOAD_IMAGE_ANYCOLOR)", imageAnyColor);
